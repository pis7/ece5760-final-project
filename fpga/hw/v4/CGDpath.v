...
module CGDpath #(
  ...
  // at bank-internal address (idx >> CLOG2_LANES).
  ...
  // without [-1:0] slices when p_lanes==1.
  ...
  // (idx[k] >> CLOG2_LANES). Because the bank index is the constant
  // genvar gi, each bank's flops only fan out to lane gi's read muxes.
  //----------------------------------------------------------------------
  ...
  generate
    for (gi = 0; gi < p_lanes; gi = gi + 1) begin : g_rd
      ...
      assign rd_a_bank_addr[gi] = rd_a_idx[gi][IDX_W-1:CLOG2_LANES];
      assign rd_b_bank_addr[gi] = rd_b_idx[gi][IDX_W-1:CLOG2_LANES];
      assign rd_c_bank_addr[gi] = rd_c_idx[gi][IDX_W-1:CLOG2_LANES];
      assign rd_d_bank_addr[gi] = rd_d_idx[gi][IDX_W-1:CLOG2_LANES];
  ...
  assign spmv_vec_bank      = spmv_vec_rd_idx[BANK_SEL_W-1:0]
                              & BANK_SEL_W'(unsigned'(p_lanes - 1));
  assign spmv_vec_bank_addr = spmv_vec_rd_idx[$clog2(p_max_n)-1:CLOG2_LANES];
  ...
  assign spmv_wb_bank = spmv_row_idx[BANK_SEL_W-1:0]
                        & BANK_SEL_W'(unsigned'(p_lanes - 1));
  assign spmv_wb_addr = spmv_row_idx[IDX_W-1:CLOG2_LANES];
  ...
  always_comb begin
    if (wr_idx_src_spmv) begin
      ...
      // SIMD or CGCtrl-scalar path: lane k's wr_idx_ctrl[k] is a linear
      // index whose low bits encode lane k by contract.
      for (int k = 0; k < p_lanes; k++) begin
        wr_bank_addr[k] = wr_idx_ctrl[k][IDX_W-1:CLOG2_LANES];
        we_eff[k]       = we[k];
      end
    end
  end
  ...
  generate
    for (gi = 0; gi < p_lanes; gi = gi + 1) begin : g_wr_addr_sec
      assign wr_bank_addr_sec[gi] = wr_idx_ctrl[gi][IDX_W-1:CLOG2_LANES];
    end
  endgenerate
