...
// it can't be cleanly stretched past 27-bit -- the SS module is fully
// parameterized and handles arbitrary widths at the cost of
// (p_wide_bits + p_frac_bits) iteration latency. CGDpath derives
...
//   5. Final multiply: m3 = |a| * r1 (48*17 = 65-bit unsigned). Computed in
//      three 16x17 partials accumulated into m3 across S_QMUL_LO/MID/HI so
//      the same 17x18 multiplier used by the NR steps is reused.
//   6. Denormalize: result_unsigned = m3 >> (49 - lzc). Saturate if any bit
//      [64:26] is set. Apply sign.
//
// Multiplier sharing: all five mult cycles (NR1_M1, NR1_M2, NR2_M1, NR2_M2,
// and the three QMUL chunks) drive a single 17x18 unsigned multiplier with
// state-driven operand muxes. This collapses what was three separate `*`
// operators (~4 DSPs total: nr_m1, nr_m2, plus a 48x17 m3 that splits) into
// one DSP, at the cost of two extra latency cycles for the QMUL chunking.
//
...
module FpDivNR #(
  ...
  // Shared 17x18 unsigned multiplier. All five mult cycles drive this one
  // physical multiplier via state-controlled operand muxes:
  //   S_NR1_M1, S_NR2_M1: b_norm * nr_input         (17 * 17 -> 34-bit)
  //   S_NR1_M2, S_NR2_M2: nr_input * nr_two_minus   (17 * 18 -> 35-bit)
  //   S_QMUL_{LO,MID,HI}: abs_a_chunk * r1          (16 * 17 -> 33-bit)
  // m3 = abs_a * r1 is reconstructed by accumulating the three QMUL chunks
  // shifted by 0, 16, 32 bits.
  logic [15:0]                  abs_a_chunk;
  always_comb begin
    case (state)
      S_QMUL_LO:  abs_a_chunk = abs_a[15:0];
      S_QMUL_MID: abs_a_chunk = abs_a[31:16];
      S_QMUL_HI:  abs_a_chunk = abs_a[47:32];
      default:    abs_a_chunk = '0;
    endcase
  end

  // Operand muxes feed a single FpMulWide instance shared across all five
  // mult cycles. The widest single-cycle product we need is 17*18 = 35
  // bits, so we widen both operands to p_total_bits-wide (zero-extended,
  // since all operands are unsigned and nonnegative) and disable the
  // fractional right shift by setting p_frac_bits=0.
  logic [p_total_bits-1:0]           mul_a;
  logic [p_total_bits-1:0]           mul_b;
  logic signed [2*p_total_bits-1:0]  mul_full_signed;
  logic [2*BNORM_W:0]                mul_full;    // 35 bits

  always_comb begin
    case (state)
      S_NR1_M1, S_NR2_M1: begin
        mul_a = p_total_bits'(b_norm);
        mul_b = p_total_bits'(nr_input);
      end
      S_NR1_M2, S_NR2_M2: begin
        mul_a = p_total_bits'(nr_input);
        mul_b = p_total_bits'(nr_two_minus);
      end
      S_QMUL_LO, S_QMUL_MID, S_QMUL_HI: begin
        mul_a = p_total_bits'(abs_a_chunk);
        mul_b = p_total_bits'(r1);
      end
      default: begin
        mul_a = '0;
        mul_b = '0;
  ...
  FpMulWide #(
    .p_int_bits  (p_total_bits),
    .p_frac_bits (0),
    .p_total_bits(p_total_bits),
    .p_wide_bits (2*p_total_bits)
  ) u_shared_mul (
    .a      ($signed(mul_a)),
    .b      ($signed(mul_b)),
    .result (mul_full_signed)
  );

  assign mul_full = (2*BNORM_W+1)'($unsigned(mul_full_signed));
  ...
  always_comb begin
    ...
    case (state)
      ...
      S_NR2_M2:                        next_state = S_QMUL_LO;
      S_QMUL_LO:                       next_state = S_QMUL_MID;
      S_QMUL_MID:                      next_state = S_QMUL_HI;
      S_QMUL_HI:                       next_state = S_QFINISH;
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      case (state)
        ...
        S_NR1_M1: begin
          // 1st NR iter, phase 1: m1 = b_norm * r0 via shared mult.
          nr_m1_reg <= mul_full[33:0];
        end
        S_NR1_M2: begin
          // 1st NR iter, phase 2: r1 = r0 * (2 - m1) via shared mult.
          r1 <= mul_full[32:16];
        end
        S_NR2_M1: begin
          // 2nd NR iter, phase 1: m1 = b_norm * r1 via shared mult.
          nr_m1_reg <= mul_full[33:0];
        end
        S_NR2_M2: begin
          // 2nd NR iter, phase 2: r1 = r1_prev * (2 - m1) via shared mult.
          r1 <= mul_full[32:16];
        end
        // m3 = abs_a * r1, computed via three 16x17 partials accumulated
        // into m3 across S_QMUL_LO/MID/HI. Each partial is shifted by the
        // chunk position before adding. Reuses the shared 17x18 mult.
        S_QMUL_LO: begin
          m3 <= M3_W'(mul_full);
        end
        S_QMUL_MID: begin
          m3 <= m3 + (M3_W'(mul_full) << 16);
        end
        S_QMUL_HI: begin
          m3 <= m3 + (M3_W'(mul_full) << 32);
        end
        S_QFINISH: begin
...
// val/rdy handshake. Fully parameterized on p_int_bits/p_frac_bits/
// p_total_bits/p_wide_bits, so this is what the FpDiv wrapper picks
// when p_total_bits > 27 (where the NR module's hard-coded 48-bit/Q1.16
// internals can't be stretched).
