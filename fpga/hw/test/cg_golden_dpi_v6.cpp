// DPI wrapper for the v6 Verilator testbench. Same layout as the v1..v5
// dpi (one contiguous flat memory shared with the DUT side), but uses
// `longint mem []` so the testbench can carry up to 64-bit fixed-point
// values uniformly. q_col / q_rowp slots store plain integers in the
// low 32 bits of their longint cell.
...
extern "C" void dpi_cg_solve(
    const svOpenArrayHandle mem,  // longint mem[total_words]
    int n,
    int max_n,
    int max_iter,
    long long eps_sq
