...
module CGTop_tb #(
  ...
  // copy in golden, since the DPI solver doesn't care about RTL
  // duplication.
  ...
  // Per-slave Avalon wires (10 slaves)
  logic [ADDR_BITS-1:0] q_val_x_addr,  q_col_x_addr,  q_rowp_x_addr;
  logic [ADDR_BITS-1:0] q_val_y_addr,  q_col_y_addr,  q_rowp_y_addr;
  logic [ADDR_BITS-1:0] cx_addr,       cy_addr;
  logic [ADDR_BITS-1:0] x_addr,        y_addr;
  logic                 q_val_x_cs,    q_col_x_cs,    q_rowp_x_cs;
  logic                 q_val_y_cs,    q_col_y_cs,    q_rowp_y_cs;
  logic                 cx_cs,         cy_cs;
  logic                 x_cs,          y_cs;
  logic                 q_val_x_clken, q_col_x_clken, q_rowp_x_clken;
  logic                 q_val_y_clken, q_col_y_clken, q_rowp_y_clken;
  logic                 cx_clken,      cy_clken;
  logic                 x_clken,       y_clken;
  logic                 q_val_x_we,    q_col_x_we,    q_rowp_x_we;
  ...
  always_ff @(posedge clk) begin
    if (q_val_x_cs && q_val_x_clken) begin
      if (q_val_x_we) q_val_x_mem[q_val_x_addr] <= q_val_x_wdata;
      else            q_val_x_rdata <= q_val_x_mem[q_val_x_addr];
    end
    if (q_col_x_cs && q_col_x_clken) begin
      if (q_col_x_we) q_col_x_mem[q_col_x_addr] <= q_col_x_wdata;
      else            q_col_x_rdata <= q_col_x_mem[q_col_x_addr];
    end
    if (q_rowp_x_cs && q_rowp_x_clken) begin
      if (q_rowp_x_we) q_rowp_x_mem[q_rowp_x_addr] <= q_rowp_x_wdata;
      else             q_rowp_x_rdata <= q_rowp_x_mem[q_rowp_x_addr];
    end
    if (q_val_y_cs && q_val_y_clken) begin
      if (q_val_y_we) q_val_y_mem[q_val_y_addr] <= q_val_y_wdata;
      else            q_val_y_rdata <= q_val_y_mem[q_val_y_addr];
    end
    if (q_col_y_cs && q_col_y_clken) begin
      if (q_col_y_we) q_col_y_mem[q_col_y_addr] <= q_col_y_wdata;
      else            q_col_y_rdata <= q_col_y_mem[q_col_y_addr];
    end
    if (q_rowp_y_cs && q_rowp_y_clken) begin
      if (q_rowp_y_we) q_rowp_y_mem[q_rowp_y_addr] <= q_rowp_y_wdata;
      else             q_rowp_y_rdata <= q_rowp_y_mem[q_rowp_y_addr];
    end
  ...
  CGTop #(
    ...
    .q_val_x_ram_address     (q_val_x_addr),
    .q_val_x_ram_chipselect  (q_val_x_cs),
    .q_val_x_ram_clken       (q_val_x_clken),
    .q_val_x_ram_write       (q_val_x_we),
    .q_val_x_ram_readdata    (q_val_x_rdata),
    .q_val_x_ram_writedata   (q_val_x_wdata),
    .q_val_x_ram_byteenable  (q_val_x_be),

    .q_col_x_ram_address     (q_col_x_addr),
    .q_col_x_ram_chipselect  (q_col_x_cs),
    .q_col_x_ram_clken       (q_col_x_clken),
    .q_col_x_ram_write       (q_col_x_we),
    .q_col_x_ram_readdata    (q_col_x_rdata),
    .q_col_x_ram_writedata   (q_col_x_wdata),
    .q_col_x_ram_byteenable  (q_col_x_be),

    .q_rowp_x_ram_address    (q_rowp_x_addr),
    .q_rowp_x_ram_chipselect (q_rowp_x_cs),
    .q_rowp_x_ram_clken      (q_rowp_x_clken),
    .q_rowp_x_ram_write      (q_rowp_x_we),
    .q_rowp_x_ram_readdata   (q_rowp_x_rdata),
    .q_rowp_x_ram_writedata  (q_rowp_x_wdata),
    .q_rowp_x_ram_byteenable (q_rowp_x_be),

    .q_val_y_ram_address     (q_val_y_addr),
    .q_val_y_ram_chipselect  (q_val_y_cs),
    .q_val_y_ram_clken       (q_val_y_clken),
    .q_val_y_ram_write       (q_val_y_we),
    .q_val_y_ram_readdata    (q_val_y_rdata),
    .q_val_y_ram_writedata   (q_val_y_wdata),
    .q_val_y_ram_byteenable  (q_val_y_be),

    .q_col_y_ram_address     (q_col_y_addr),
    .q_col_y_ram_chipselect  (q_col_y_cs),
    .q_col_y_ram_clken       (q_col_y_clken),
    .q_col_y_ram_write       (q_col_y_we),
    .q_col_y_ram_readdata    (q_col_y_rdata),
    .q_col_y_ram_writedata   (q_col_y_wdata),
    .q_col_y_ram_byteenable  (q_col_y_be),

    .q_rowp_y_ram_address    (q_rowp_y_addr),
    .q_rowp_y_ram_chipselect (q_rowp_y_cs),
    .q_rowp_y_ram_clken      (q_rowp_y_clken),
    .q_rowp_y_ram_write      (q_rowp_y_we),
    .q_rowp_y_ram_readdata   (q_rowp_y_rdata),
    .q_rowp_y_ram_writedata  (q_rowp_y_wdata),
    .q_rowp_y_ram_byteenable (q_rowp_y_be),
  ...
  task automatic load_test(
    ...
    // Per-slave loads (Q duplicated: same data into both x and y trios)
    for (int j = 0; j < nnz; j++) begin
      q_val_x_mem[j] = vals[j];
      q_col_x_mem[j] = col_idx[j];
      q_val_y_mem[j] = vals[j];
      q_col_y_mem[j] = col_idx[j];
    end
    for (int i = 0; i <= t_n; i++) begin
      q_rowp_x_mem[i] = row_ptr[i];
      q_rowp_y_mem[i] = row_ptr[i];
    end
    for (int i = 0; i < t_n; i++) begin
      cx_mem[i] = cx[i];
      cy_mem[i] = cy[i];
    ...
    // Golden mirror (v4 contiguous layout, single Q copy)
    for (int j = 0; j < nnz; j++) begin
