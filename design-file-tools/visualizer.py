"""Visualize LEF/DEF placement using Tk.

Usage: uv run design-file-tools/visualizer.py <benchmark_dir>
  e.g. uv run design-file-tools/visualizer.py benchmarks/iccad04/DMA
...
class PlacementViewer:
    """Tk canvas viewer for LEF/DEF placement data with pan and zoom."""

    CANVAS_W = 1200
    CANVAS_H = 900
    PAD = 40  # pixel padding around die area

    def __init__(self, lef: LefParser, defn: DefParser) -> None:
        self.lef: LefParser = lef
        self.defn: DefParser = defn
        self.dbu_per_micron: int = defn.dbu_per_micron

        # Die bounds in DBU
        self.die_xl: float = defn.die_area[0]
        self.die_yl: float = defn.die_area[1]
        self.die_xh: float = defn.die_area[2]
        self.die_yh: float = defn.die_area[3]
        ...
        self.root.title(f"Placement: {defn.design_name}")
        ...
        self._update_status()
        # Defer initial fit until canvas is mapped and has its real size
    ...
    def _to_canvas(self, dbu_x: float, dbu_y: float) -> tuple[float, float]:
        ...
        # Flip y-axis: DEF y increases upward, canvas y increases downward
    ...
    def _draw_components(self) -> None:
        ...
        for comp in self.defn.components.values():
            macro = self.lef.macros.get(comp.macro_name)
            if macro is None:
                continue
            # Macro size is in microns, convert to DBU
            w = macro.width * self.dbu_per_micron
            h = macro.height * self.dbu_per_micron
            x0, y0 = self._to_canvas(comp.x, comp.y)
            x1, y1 = self._to_canvas(comp.x + w, comp.y + h)
            self.canvas.create_rectangle(x0, y0, x1, y1, fill="#4080c0", outline="")

    def _draw_io_pins(self) -> None:
        """Draw I/O pins as small diamonds."""
        die_size = min(self.die_xh - self.die_xl, self.die_yh - self.die_yl)
        r = max(3, die_size * 0.008 * self.scale)
        for pin in self.defn.io_pins:
    ...
    def _update_status(self) -> None:
        ...
        n_comp = len(self.defn.components)
        n_pins = len(self.defn.io_pins)
        n_nets = len(self.defn.nets)
        n_macros = len(self.lef.macros)
        self.status.config(
            text=(
                f"  {self.defn.design_name}  |  "
                f"{n_macros} macros, {n_comp} components, "
                f"{n_pins} I/O pins, {n_nets} nets  |  "
    ...
    def _on_resize(self, event: tk.Event) -> None:  # type: ignore[type-arg]
        pass  # could auto-refit, but that fights manual pan/zoom
...
def main() -> None:
    ...
    lef_path, def_path = find_lef_def(sys.argv[1])
    print(f"LEF: {lef_path}")
    print(f"DEF: {def_path}")

    lef = LefParser(lef_path)
    defn = DefParser(def_path)
    print(f"Design: {defn.design_name}")
    print(f"  {len(lef.macros)} macros, {len(defn.components)} components, "
          f"{len(defn.io_pins)} I/O pins, {len(defn.nets)} nets")

    viewer = PlacementViewer(lef, defn)
    viewer.show()
