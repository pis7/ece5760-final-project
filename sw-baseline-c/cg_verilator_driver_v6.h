...
//
// TOTAL_BITS defaults to 27 to match the FPGA bitstream; verilated
// builds may override via HW_TOTAL_BITS up to 64. When TOTAL_BITS<=32
// Verilator emits the value-port signals as 32-bit IData; >32 they
// become 64-bit QData. The driver uniformly snapshots them as
// uint64_t and stores into int64_t mirrors -- the implicit narrowing
...
public:
    ...
    void load_q_initial(const CSRMatrix& src) {
        ...
        for (int j = 0; j < nnz; ++j) {
            int64_t v = double_to_fp(Q.vals[j]);
...
private:
    ...
    // 64-bit value-port service. Verilator emits the bus as IData
    // (uint32_t) for TOTAL_BITS<=32 and QData (uint64_t) for >32; both
    // implicitly convert to/from uint64_t here.
        ...
        int64_t* mem, int depth, uint64_t* readdata_out)
    {
        if (cs_clken && addr < static_cast<uint32_t>(depth)) {
            if (wr) {
                mem[addr] = static_cast<int64_t>(wdata);
            } else {
                *readdata_out = static_cast<uint64_t>(mem[addr]);
            }
        }
    }

    // 32-bit index-port service (q_col / q_rowp).
    ...
    void tick() {
        ...
        // Snapshot pre-edge port state for each of the ten slaves. Value
        // ports use uint64_t snapshots; index ports stay uint32_t.
        ...
        uint64_t q_val_x_wd   = dut_->q_val_x_ram_writedata;
        ...
        uint64_t q_val_y_wd   = dut_->q_val_y_ram_writedata;
