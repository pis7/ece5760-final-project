...
public:
    ...
    void load_q_initial(const CSRMatrix& src) {
        ...
        for (int j = 0; j < nnz; ++j) {
            q_val_mem_[j] = double_to_fp(Q.vals[j]);
            q_col_mem_[j] = Q.col_idx[j];
        }
        for (int i = 0; i <= Q.n; ++i)
            q_rowp_mem_[i] = Q.row_ptr[i];
    }
...
private:
    ...
    void tick() {
        ...
        bool     q_col_wr   = dut_->q_col_ram_write;
        uint32_t q_col_wd   = dut_->q_col_ram_writedata;

        bool     q_rowp_cs  = dut_->q_rowp_ram_chipselect && dut_->q_rowp_ram_clken;
        uint32_t q_rowp_a   = dut_->q_rowp_ram_address;
        bool     q_rowp_wr  = dut_->q_rowp_ram_write;
        uint32_t q_rowp_wd  = dut_->q_rowp_ram_writedata;
        ...
        dut_->q_rowp_ram_readdata = q_rowp_rd;
